...
### c) Spring Boot 4.x AOP 의존성 오류 
수업시간에 aop 의존성 설치가 안 되는 문제가 발생하여 원인을 찾아봤다.

```plaintext
Could not find org.springframework.boot:spring-boot-starter-aop:.
Required by:
    root project 'my_api_server'
```

* **문제**: `build.gradle`에서 `spring-boot-starter-aop` 의존성 resolve 실패
* **원인**: Spring boot 4.0 업데이트로 인한 artifact 이름 변경
* **해결법**: Spring Boot 4.0부터 `spring-boot-starter-aop` → `spring-boot-starter-aspectj` 사용
    ```gradle
    // ❌ 
...
`spring-boot-gradle-plugin`이 BOM을 통해 현재 Boot 버전에 맞는 artifact를 자동으로 resolve하는데,
Maven Central에 `spring-boot-starter-aop` 4.0.x GA 버전이 존재하지 않아 아래 에러가 발생한다.


> 참고: [Spring Boot 4.0 Migration Guide - AOP Starter POM](https://github.com/spring-projects/spring-boot/wiki/Spring-Boot-4.0-Migration-Guide#aop-starter-pom)